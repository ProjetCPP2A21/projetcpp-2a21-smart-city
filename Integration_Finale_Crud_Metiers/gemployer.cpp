...
void MainWindow::onLoginClicked()
{
    ...
    // --- CORRECTION ICI ---
    // On convertit le String en Int pour que la comparaison SQL fonctionne
    query.bindValue(":id", username.toInt());

    if (!query.exec()) {
        QMessageBox::critical(this, "Erreur", "Erreur de connexion !");
        // Ajoutez ceci pour voir l'erreur technique si ça plante
        qDebug() << "Erreur SQL :" << query.lastError().text();
        return;
    }

    // Debug : Regardez la console "Application Output" pour voir ce qui est envoyé
    qDebug() << "Login tentative - ID:" << username << " Int:" << username.toInt();
...
void MainWindow::on_Ajouter_Employe_Clicked()
{
    ...
    QString sexe = ui->Statistiques_Employe->text();
