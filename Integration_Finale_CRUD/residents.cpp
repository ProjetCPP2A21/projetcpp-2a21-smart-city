...
{
    QSqlQuery query;
    query.prepare("INSERT INTO RESIDENT (ID_RESIDENT, ID_RESIDENCE, NOM, PRENOM, AGE, SEXE, PROFESSION, SITUATION_FAMILIALE) "
...
QSqlQueryModel* Resident::afficher()
{
    ...
    model->setQuery("SELECT ID_RESIDENT, ID_RESIDENCE, NOM, PRENOM, AGE, SEXE, PROFESSION, SITUATION_FAMILIALE FROM RESIDENT");
    return model;
}
...
{
    QSqlQuery query;
    query.prepare("UPDATE RESIDENT SET ID_RESIDENCE=:id_residence, NOM=:nom, PRENOM=:prenom, AGE=:age, SEXE=:sexe, "
                  "PROFESSION=:profession, SITUATION_FAMILIALE=:situation_familiale WHERE ID_RESIDENT=:id_resident");
...
{
    QSqlQueryModel* model = new QSqlQueryModel();
    model->setQuery("SELECT * FROM RESIDENT WHERE NOM LIKE '%" + texte + "%' OR PRENOM LIKE '%" + texte + "%' OR SEXE LIKE '%" + texte + "%'");
    return model;
}
...
{
    QSqlQueryModel* model = new QSqlQueryModel();
    QString query = "SELECT * FROM RESIDENT ORDER BY " + critere + " " + ordre;
