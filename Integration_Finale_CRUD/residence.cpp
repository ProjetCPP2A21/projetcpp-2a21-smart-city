...
{
    bool okId = false;
    int id = ui->ID_Residence->text().toInt(&okId);
    if (!okId) {
...
{
    // Récupérer l'ID à partir du bon widget (ex: ID_Residence) — NE PAS UTILISER Nom_Residence
    bool ok = false;
    int id = ui->ID_Residence->text().toInt(&ok); // <-- s'assurer qu'on lit le champ ID
    if (!ok || id <= 0) {
        QMessageBox::warning(this, "Erreur", "ID invalide. Vérifiez que vous avez saisi un entier > 0.");
        return;
    }

    // Debug avant suppression
    qDebug() << "Suppression demandée pour ID =" << id;
