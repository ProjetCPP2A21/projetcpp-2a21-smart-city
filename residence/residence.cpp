    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->pushButton_alarme, &QPushButton::clicked,
            this, &MainWindow::checkTemperatureAndAlert);
...
void MainWindow::on_statistique_clicked()
{
    ...
    chartView->setParent(ui->chart);        // Make it a child of your widget
    chartView->setGeometry(ui->chart->rect()); // Fill the entire widget
    chartView->show();
}
...
void MainWindow::on_exporter_clicked()
{
    ...
    // 1️⃣ Get data directly from database
    ...
    // 2️⃣ Fill HTML table with database data
    while (query.next()) {
        html += "<tr>";
        for (int col = 0; col < 8; ++col) {
...
void MainWindow::checkTemperatureAndAlert()
{
    qDebug() << "checkTemperatureAndAlert appelé"; // trace l'appel du bouton

    QString idString = ui->lineEdit_id_arduino->text().trimmed();
    qDebug() << "ID saisi :" << idString;

    if (idString.isEmpty()) {
        qDebug() << "Aucun ID saisi !";
        return;
    }
    ...
    QSqlQuery query;
    query.prepare("SELECT TEMPERATURE FROM RESIDENCE WHERE ID_RESIDENCE = :id");
    query.bindValue(":id", id);
    ...
    if (!query.next()) {
        qDebug() << "Aucun bâtiment trouvé avec cet ID.";
        return;
    }

    float temp = query.value(0).toFloat();
    qDebug() << "Température lue pour ID =" << id << " → " << temp;

    if (temp > 50) {
        A.write_to_arduino("3");
        qDebug() << "3 bips envoyés";
    } else if (temp > 40) {
        A.write_to_arduino("2");
        qDebug() << "2 bips envoyés";
    } else if (temp > 30) {
        A.write_to_arduino("1");
        qDebug() << "1 bip envoyé";
    } else {
        A.write_to_arduino("0");
        qDebug() << "Temp normale → aucun bip";
    }
}
