...
void MainWindow::afficherServices()
{
    ...
    // Définir le nombre de colonnes et leurs en-têtes
    ui->tableServices->setColumnCount(7);
    QStringList headers;
    headers << "Nom" << "Type" << "Responsable" << "Budget"
            << "Priorité" << "État" << "ID";
    ui->tableServices->setHorizontalHeaderLabels(headers);

    // (Optionnel) cacher la colonne ID si tu veux qu’elle soit invisible
    // ui->tableServices->setColumnHidden(6, true);
