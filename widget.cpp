...
Widget::Widget(QWidget *parent)
    ...
    , ui(new Ui::Widget)
{
    // Initialiser l'interface graphique définie dans widget.ui
    ui->setupUi(this);

    // Afficher tous les services au démarrage de l'application
    afficherServices();

    // ========== CONNEXION DES BOUTONS ==========
    // Quand l'utilisateur clique sur un bouton, appeler la fonction correspondante

    // Bouton "Ajouter" → appeler ajouterService()
    connect(ui->ajouterButton, &QPushButton::clicked,
            this, &Widget::ajouterService);

    // Bouton "Modifier" → appeler modifierService()
    connect(ui->modifierButton, &QPushButton::clicked,
            this, &Widget::modifierService);

    // Bouton "Supprimer" → appeler supprimerService()
    connect(ui->supprimerButton, &QPushButton::clicked,
            this, &Widget::supprimerService);

    // Bouton "Rechercher" → appeler rechercherService()
    connect(ui->pushButton_9, &QPushButton::clicked,
            this, &Widget::rechercherService);

    // ========== CONNEXION DU TRI ==========
    // Quand l'utilisateur change la sélection du ComboBox de tri
    connect(ui->combo_sort, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &Widget::trierServices);

    // ========== CONNEXION DU TABLEAU ==========
    // Quand l'utilisateur clique sur une ligne du tableau
    connect(ui->tableServices, &QTableWidget::cellClicked,
            this, &Widget::selectionnerService);
}
 ...
 */
void Widget::ajouterService()
{
    // ========== RÉCUPÉRATION DES DONNÉES DU FORMULAIRE ==========

    QString nom = ui->lineEdit_name->text();              // Champ "Nom"
    QString type = ui->combo_type->currentText();         // ComboBox "Type"
    QString responsable = ui->lineEdit_responsable->text();// Champ "Responsable"
 ...
 */
void Widget::selectionnerService(int row, int column)
{
    ...
    ui->lineEdit_id->setText(id);                    // Champ ID
    ui->lineEdit_name->setText(nom);                 // Champ Nom
    ui->combo_type->setCurrentText(type);            // ComboBox Type
    ui->lineEdit_responsable->setText(responsable);  // Champ Responsable
 ...
 */
void Widget::trierServices(int index)
{
 ...
 */
void Widget::viderChamps()
{
    ui->lineEdit_id->clear();                        // Vider l'ID
    ui->lineEdit_name->clear();                      // Vider le nom
    ui->lineEdit_responsable->clear();               // Vider le responsable
    ui->combo_type->setCurrentIndex(0);              // Réinitialiser le type
